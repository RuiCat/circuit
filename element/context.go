...
func (con *Context) CallMark(mark Mark) {
	...
	case MarkStartIteration:
		for _, v := range con.Nodelist {
			ElementLitt[v.Base().NodeType].StartIteration(con, con.Time, v)
		}
	case MarkStamp:
		for _, v := range con.Nodelist {
			ElementLitt[v.Base().NodeType].Stamp(con, con.Time, v)
		}
	case MarkDoStep:
		for _, v := range con.Nodelist {
			ElementLitt[v.Base().NodeType].DoStep(con, con.Time, v)
		}
	case MarkCalculateCurrent:
		for _, v := range con.Nodelist {
			ElementLitt[v.Base().NodeType].CalculateCurrent(con, con.Time, v)
		}
	case MarkStepFinished:
		for _, v := range con.Nodelist {
			ElementLitt[v.Base().NodeType].StepFinished(con, con.Time, v)
		}
	default:
