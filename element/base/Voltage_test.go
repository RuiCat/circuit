...
func TestVoltageSource(t *testing.T) {
	...
	t.Run("DCVoltage", func(t *testing.T) {
		...
		// 设置引脚
		ele[0].SetNodePins(0, -1) // 电压源正极接节点0，负极接地
		ele[1].SetNodePins(0, -1) // 电阻一端接节点0，另一端接地
		...
		if err := time.TransientSimulation(timeMNA, mnaSolver, ele, func(voltages []float64) {}); err != nil {
			t.Fatalf("仿真失败 %s", err)
		}
