...
func handleLoadFP_D(vmst *VmState, ir uint32, pc uint32) (uint32, uint32, uint32, VmMcauseCode) {
	// --- 解码 ---
	rd := (ir >> 7) & 0x1f
	rs1 := (ir >> 15) & 0x1f
...
func handleStoreFP_D(vmst *VmState, ir uint32, pc uint32) (uint32, uint32, uint32, VmMcauseCode) {
	// --- 解码 ---
	rs1 := (ir >> 15) & 0x1f
	rs2 := (ir >> 20) & 0x1f
	// S-Type 立即数重组
	...
	// --- 存入内存 ---
	if !vmst.PutUint64(addr, val_bits) {
		vmst.Core.Mtval = addr
